...
<style>
  body{ font-family: Verdana,Arial; margin-left: 1%; margin-right: 1%; margin-top: 20px; }
  td{ font-size: 75%; }
  th{ font-size: 70%; font-weight: bolder; border: 1px solid lightgrey; vertical-align: bottom; }
  hr{ border:1px solid #333; margin:0px 0; }
  a{ cursor:hand; }
  ...
  .block{ border: solid #333 1px; width: 100%; }
  .block2{ border: 0px; width: 100%; }
  .thin{ border:1px solid #333; height:1px;}
...
<!-- ********** SUMMARY SECTION ********** -->
  ...
  <tr>
    ...
    <table class="block table-condensed" cellspacing='3px'>
    <tr>
...
<xsl:for-each select="msxsl:node-set($topics)/topic">
  <xsl:if test="ms:unique( msxsl:node-set($topics)/topic, number(position()) )">
    <xsl:variable name="topic" select="."/>
    <table class="" width="100%" style="border: solid gray 1px;">
      <tr><td>
        <table width="100%"><tr>
          <td class="h2 b2">
            <xsl:call-template name="label">
...
<table class="block2" cellspacing='3px'>
  <tr>
    <td>
      <table class='' width='100%'>
        <tr>
    ...
    <tfoot>
        <tr>
          ...
          <xsl:for-each select="$header[data[@summary]]/data">
            <td>
            <xsl:if test="@summary">
              <xsl:attribute name="style">border-top: 1px solid #333;</xsl:attribute>
            </xsl:if>
    ...
    <xsl:otherwise>
      <div style="margin-left:3;position:absolute;width:8;height:10;background:infobackground;">
        <div style="position:absolute;top:-5;left:-3;">
          <font style="font-size: 11pt;font-weight:normal;" color="#333" face="webdings"><xsl:value-of select="'&#x9D;'"/></font>
        </div>
      </div>
    </xsl:otherwise>
    </xsl:choose>
...
<xsl:param name="state"/>
  <div style="border: solid #333 1px; position: relative; width: 11px; height: 11px;">
